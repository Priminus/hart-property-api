-- Fix URA merge functions to work with new partial indexes
-- When URA ingests, it should only target rows where exact_level/unit are NULL (the generic rows)
...
BEGIN
  ...
  -- We ONLY update rows where exact_level is NULL to avoid corrupting manual unit data
  ...
  IF NOT FOUND THEN
    ...
    ON CONFLICT (condo_name_lower, sale_price, sale_month) 
