-- Update the batch merge function to also fill property_type for rows WITH exact_level
-- This allows URA data to enrich OCR-imported rows that have exact_level/exact_unit but missing property_type
...
BEGIN
  ...
  LOOP
    ...
    -- Try to update existing generic row (no exact_level) - full merge
    ...
    ELSE
      -- Also try to fill property_type for rows WITH exact_level (OCR-imported rows)
      -- Only update property_type if it's NULL - don't touch other fields
      UPDATE sale_transactions
...
BEGIN
  ...
  IF NOT v_updated THEN
    -- Also try to fill property_type for rows WITH exact_level
    UPDATE sale_transactions
